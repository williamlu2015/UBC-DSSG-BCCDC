...
from src.analysis.ml_trials.feature_engineering import change_organisms, \
    change_all, split_by_organism_better, change_numbers, \
...
from src.analysis.ml_trials.splitting import split_skewed, split_by_organism, \
    split_by_date_half, split_by_date_quarter
...
SAVE_TO = from_root("results\\test_outcome\\2_class")
...
def run_feature_engineering_trials():
    ...
    df = extract(from_root("sql\\test_outcome\\2_class_metamap.sql"))
    change_organisms(
        df, OUTPUT, LABELS, os.path.join(SAVE_TO, "change_organisms"))
    change_all(
        df, OUTPUT, LABELS, os.path.join(SAVE_TO, "change_all"))
    split_by_organism_better(
        df, LABELS, os.path.join(SAVE_TO, "split_by_organism_better"))
    change_numbers_organisms_kmeans(
        df, OUTPUT, 2, os.path.join(SAVE_TO, "change_numbers_organisms_kmeans"))
...
def run_splitting_trials():
    ...
    split_by_organism(df, LABELS, os.path.join(SAVE_TO, "split_by_organism"))

    df = extract(from_root("sql\\test_outcome\\2_class_date.sql"))
    split_by_date_half(
...
def run_clustering_trials():
    ...
    cluster_labelled(df, OUTPUT, 2, os.path.join(SAVE_TO, "cluster_labelled"))
    cluster_labelled_1000_features(
        df, OUTPUT, 2, os.path.join(SAVE_TO, "cluster_labelled_1000_features"))
    cluster_labelled_1000_features_pca(
        df, OUTPUT, 2,
        os.path.join(SAVE_TO, "cluster_labelled_1000_features_pca"))
    cluster_labelled_tfidf(
        df, OUTPUT, 2, os.path.join(SAVE_TO, "cluster_labelled_tfidf"))
    cluster_labelled_variance(
        df, OUTPUT, 2, os.path.join(SAVE_TO, "cluster_labelled_variance"))
