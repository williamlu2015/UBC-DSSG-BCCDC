...
SAVE_TO = from_root("results\\test_outcome\\3_class")
...
def run_basic_trials():
    df = extract(from_root("sql\\test_outcome\\3_class.sql"))
...
def run_feature_engineering_trials():
    ...
    df = extract(from_root("sql\\test_outcome\\3_class_metamap.sql"))
    change_organisms(
    ...
    change_numbers_organisms_kmeans(
        df, OUTPUT, 3, os.path.join(SAVE_TO, "change_numbers_organisms_kmeans"))
...
def run_splitting_trials():
    ...
    df = extract(from_root("sql\\test_outcome\\3_class_date.sql"))
    split_by_date_half(
...
def run_clustering_trials():
    ...
    cluster_labelled(df, OUTPUT, 3, os.path.join(SAVE_TO, "cluster_labelled"))
    cluster_labelled_1000_features(
        df, OUTPUT, 3, os.path.join(SAVE_TO, "cluster_labelled_1000_features"))
    ...
    cluster_labelled_tfidf(
        df, OUTPUT, 3, os.path.join(SAVE_TO, "cluster_labelled_tfidf"))
    cluster_labelled_variance(
        df, OUTPUT, 3, os.path.join(SAVE_TO, "cluster_labelled_variance"))
